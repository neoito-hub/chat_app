...
import { RequestBody, TemplateInfo, ProjectInfo, ChatInfo, Param, VendorDetails, UserInfo } from './interface.ts'
    ...
    await prisma.$transaction(async (prisma) => {
      ...
      for (const recipient of actualRecipientIds) {
        ...
        const projectInfo: ProjectInfo[] =
          await prisma.$queryRaw`select * from projects where id= ${reqBody.project_id}`

        const candidateInfo: any[] = await prisma.$queryRaw`select * from contacts as c where c.id =${recipient}`
        ...
        const getValidatedPhoneNumber = await validatePhoneNumber(
          candidateInfo[0].phone_number,
          candidateInfo[0].country_code
        )
        ...
        const projectDetails: ProjectInfo = await prisma.projects.findFirst({
          where: {
