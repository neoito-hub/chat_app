...
const handler = async event => {
  ...
  try {
    ...
    const getValidatedPhoneNumber = await validatePhoneNumber(reqBody.candidate_details.phone_number, reqBody.candidate_details.country_code);
...
`;
    const candidateInfo = await prisma.$queryRaw`select * from contacts as c where c.phone_number =${reqBody?.candidate_details?.phone_number}`;
    ...
    if (!contactExist) {
      ...
      await prisma.$transaction(async prisma => {
        ...
        let message = await axios.post(`${baseURL}${api_version}/${phoneNumber_id}/messages`, {
