...
interface NewClientTemplateMessage {
  id: string
}
...
const handler = async (event: { req: Request; res: Response }): Promise<void> => {
  ...
  try {
    ...
    const result: Result = await prisma.$transaction(async (tx) => {
      let headerObject: HeaderObject | null = null
      let footerObject: FooterObject | null = null
      let bodyObject: BodyObject | null = null
      ...
      if (
        ...
        templateDetails.language_name !== reqBody.language ||
        templateDetails.template_type !== reqBody.type ||
        templateDetails.language_id !== reqBody.languageId ||
        templateDetails.category_id !== reqBody.categoryId
      ) {
        return sendResponse(res, 400, {
      ...
      if (templateDetails.updatedAt && templateDetails.status === 'APPROVED') {
        ...
        if (diffInTime.asHours() < 24) {
          ...
          templateDetails.updated_at = new Date()
      ...
      try {
        let newClientTemplateMessage: NewClientTemplateMessage = await tx.templates.update({
        ...
        if (buttonArray.length > 0) {
          await tx.template_message_buttons.delete({
            where: { template_message_id: reqBody.template_id }, // Assuming `reqBody.id` contains the ID of the template to delete
          })
        ...
        if (headerObject != null) {
          if (
            ...
            let matchResult = stringifiedHeaderObject.match(pattern)
            let extractedHeaderParams: string[]
            if (matchResult) {
              extractedHeaderParams = [...matchResult]
              extractedHeaderParams = [...new Set(extractedHeaderParams)]
            } else {
              let extractedBodyParams: string[] = []
            }
            ...
            let parsedHeaderObject: ParsedHeaderObject = JSON.parse(stringifiedHeaderObject)
        ...
        if (bodyObject != null) {
          ...
          let matchResult = stringifiedBodyObject.match(pattern)
          let extractedBodyParams: string[]
          if (matchResult) {
            extractedBodyParams = [...matchResult]
          ...
          if (bodyParams.length != 0) {
            await tx.template_message_custom_parameters.deleteMany({
              where: {
                template_message_id: reqBody.template_id,
              },
            })
            await tx.template_message_custom_parameters.createMany({
              data: bodyParams,
          ...
          let parsedBodyObject: ParsedBodyObject = JSON.parse(stringifiedBodyObject)
        ...
        let templateCreationResponse: TemplateCreationResponse | null = null

        templateCreationResponse = await axios
          .post(`${baseURL}v17.0/${templateDetails.template_uuid}`, reqData, auth_config)
          .then((result) => {
            return result
          })
          .catch((error) => {
