...
const handler = async event => {
  ...
  try {
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      let headerUrl = null;
      if (templateDetails.name !== reqBody.name || templateDetails.language_name !== reqBody.language || templateDetails.template_type !== reqBody.type || templateDetails.language_id !== reqBody.languageId || templateDetails.category_id !== reqBody.categoryId) {
        return sendResponse(res, 400, {
      ...
      try {
        ...
        if (buttonArray.length > 0) {
          await tx.template_message_buttons.delete({
            ...
            } // Assuming `reqBody.id` contains the ID of the template to delete
          });
        ...
        templateCreationResponse = await axios.post(`${baseURL}v17.0/${templateDetails.template_uuid}`, reqData, auth_config).then(result => {
          return result;
        }).catch(error => {
          return error;
        });
