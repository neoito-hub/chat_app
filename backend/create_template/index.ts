...
import { Component, Button, Buttonbody, TemplateRequestBody, UserInfo, TemplateCreationResponse } from './interface.ts'
...
const handler = async (event: { req: Request; res: Response }): Promise<void> => {
  ...
  try {
    ...
    const reqBody: TemplateRequestBody = await getBody(req)
    ...
    const result: TemplateCreationResponse = await prisma.$transaction(async (tx) => {
      let headerObject: Component | null = null
      let footerObject: Component | null = null
      let bodyObject: Component | null = null
      let buttonArray: Button[] = []
      let modifiedComponentsArray: Component[] = []
      ...
      try {
        ...
        if (headerObject !== null) {
          if (
            ...
            let extractedHeaderParams: RegExpMatchArray | null = stringifiedHeaderObject.match(pattern)
            if (extractedHeaderParams) {
              extractedHeaderParams = Array.from(new Set(extractedHeaderParams)) as RegExpMatchArray
            }
        ...
        if (bodyObject !== null) {
          ...
          let extractedBodyParams: RegExpMatchArray | null = stringifiedBodyObject.match(pattern)
          if (extractedBodyParams) {
            extractedBodyParams = Array.from(new Set(extractedBodyParams)) as RegExpMatchArray
          }
          let paramIncrementor = 1
          let bodyParams = []
          let exampleArray = []
        ...
        let templateCreationResponse: TemplateCreationResponse = null

        templateCreationResponse = await axios
          .post(`${baseURL}${vendorDetails.vendor_api_version}/${businessId}/message_templates`, reqBody, auth_config)
          .then((result) => {
            return result
          })
          .catch((error) => {
